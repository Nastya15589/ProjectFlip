...
class CartPage(Base):
    ...
    empty_cart_message = "//p[contains(text(), 'В Вашей корзине пусто :(')]"
    ...
    # Actions
    def get_name_model_cart(self):      # Получаем Наименование модели товара в корзине
        name_model_cart = self.get_name_model().text.split(": ")[1]
        print(f'Название модели в корзине:{name_model_cart}')
        return name_model_cart

    def get_brand_cart(self):       # Получаем Производителя товара в корзине
        brand_cart = self.get_brand().text.split(": ")[1]
        print(f'Производитель товара в корзине:{brand_cart}')
        return brand_cart

    def get_name_product_cart(self):        # Получаем Наименование товара в корзине
    ...
    def get_product_size_cart(self):        # Получаем Размер товара в корзине
    ...
    def get_price_cart(self):       # Получаем Цену товара в корзине
        price_cart = self.get_price().text
        print(f'Цена товара в корзине:{price_cart}')
        return price_cart

    def click_submit_button(self):      # Нажимаем на кнопку оформления заказа
        self.get_submit_button().click()
        print("Click submit button")

    def check_checkbox(self):      # Нажали на чекбокс
        self.get_checkbox().click()
        print("Check checkbox")

    def click_delete_product(self):      # Нажимаем на кнопку удалить
    ...
    def product_information_in_cart(self):
        ...
        self.assert_word(self.get_title_page(), "Корзина")
        name_product_cart = self.get_name_product_cart()
        brand_cart = self.get_brand_cart().title()
        model_cart = self.get_name_model_cart()
        product_size_cart = self.get_product_size_cart()
        price_cart = self.get_price_cart()
    ...
    def delete_product(self):
        ...
        self.assert_word(self.get_empty_cart_message(), "В Вашей корзине пусто :(")
