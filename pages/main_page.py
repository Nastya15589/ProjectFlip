...
class MainPage(Base):
    ...
    # Actions
    def click_catalog_women_clothing(self):     # Нажимаем на раздел Женской одежды
        self.get_catalog_women_clothing().click()
        print("Click catalog women clothing")

    def move_to_catalog_of_clothes_shoes_accessories(self):     # Наводимся на раздел Одежда, обувь и аксессуары
    ...
    def click_cart(self):     # Нажимаем на Корзину
        self.get_cart().click()
        print("Click cart")

    # Methods
    def select_catalog_section(self):
        self.move_to_catalog_of_clothes_shoes_accessories()
        self.click_catalog_women_clothing()
        self.assert_word(self.get_name_section(), "Женская одежда")
